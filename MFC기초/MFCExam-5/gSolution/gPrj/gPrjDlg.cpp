...
BEGIN_MESSAGE_MAP(CgPrjDlg, CDialogEx)
	...
	ON_WM_QUERYDRAGICON()
	ON_WM_DESTROY()
	ON_BN_CLICKED(IDC_BTNTEST, &CgPrjDlg::OnBnClickedBtntest)
	ON_BN_CLICKED(IDC_BTNPROCESS, &CgPrjDlg::OnBnClickedBtnprocess)
	ON_BN_CLICKED(IDC_BTNMAKE_PATTERN, &CgPrjDlg::OnBnClickedBtnmakePattern)
...
BOOL CgPrjDlg::OnInitDialog()
{
	...
	m_pDlgImgReuslt = new CDlgImage;
	m_pDlgImgReuslt->Create(IDD_DLGIMAGE, this);
	m_pDlgImgReuslt->ShowWindow(SW_SHOW);
	m_pDlgImgReuslt->MoveWindow(640, 0, 640, 480);
...
void CgPrjDlg::OnBnClickedBtntest()
{
	...
	for (int j = 0; j < nHeight; j++) {
		for (int i = 0; i < nWidth; i++) {
			if(fm[j * nPitch + i] > nth){
				if (m_pDlgImgReuslt->m_nDataCount < MAX_POINT) {
					cout << nindex << ":" << i << "," << j << endl;
					m_pDlgImgReuslt->m_ptData[nindex].x = i;
					m_pDlgImgReuslt->m_ptData[nindex].y = j;
					m_pDlgImgReuslt->m_nDataCount = ++nindex;
	...
	m_pDlgImgReuslt->Invalidate();

}

#include <chrono> // 처리 시간 확인
void CgPrjDlg::OnBnClickedBtnprocess()
{
	...
	cout << nRet << "\t" << millisec.count() << "ms" << endl;	//\t 탭
...
void CgPrjDlg::OnBnClickedBtnmakePattern()
{
	...
	CRect rect(100, 200, 150, 400); // 범위 설정
