...
BEGIN_MESSAGE_MAP(CmfcCimageDlg, CDialogEx)
	...
	ON_WM_PAINT()
	ON_WM_QUERYDRAGICON()
	ON_BN_CLICKED(IDC_BTNIMAGE, &CmfcCimageDlg::OnBnClickedBtnimage)
	ON_BN_CLICKED(IDC_BTNSAVE, &CmfcCimageDlg::OnBnClickedBtnsave)
	ON_BN_CLICKED(IDC_BTNLOAD, &CmfcCimageDlg::OnBnClickedBtnload)
	ON_BN_CLICKED(IDC_BTNACTION, &CmfcCimageDlg::OnBnClickedBtnaction)
END_MESSAGE_MAP()
...
void CmfcCimageDlg::OnBnClickedBtnimage()
{
	...
	m_image.Create(nWidth, -nHeight, nBpp); // header파일에 생성
	if (nBpp == 8) {
	...
	//		fm[j * nPitch + i] = 0; // Gridation

	//		//fm[j * nPitch + i] = j%255; // Gridation
	//	}
	//}
...
void CmfcCimageDlg::OnBnClickedBtnsave()
{
	...
	m_image.Save(g_strFileImage); // file save
}
...
void CmfcCimageDlg::UpdateDisplay() {
	CClientDC dc(this); // this라는 다이얼로그
	m_image.Draw(dc, 0, 0);
}
...
void CmfcCimageDlg::moveRect() {
	...
	//memset(fm, 0xff, sizeof(unsigned char) * nWidth * nHeight);	// 다시 그리는 함수
...
BOOL CmfcCimageDlg::validImagPos(int x, int y){
	int nWidth = m_image.GetWidth();
	int nHeight = m_image.GetHeight();
